...
class ThermalFluidControl(LBMEnv):
    ...
    def __init__(self, params=DEFAULT_PARAMS, render_mode=None):
    ...
    def initialize_sim(self) -> lbm.Simulation:
        ...
        # Define the domain
        domain = lbm.Domain(shape=(self.nx, self.ny), bounds=(0.0, 1.0, 0.0, 1.0))
        ...
        sim.set_boundary_conditions(
            lbm.BounceBackBoundary("bottom", sim.bottom), "ThermalLattice"
        )
        sim.set_boundary_conditions(
    ...
    def step(self, action):
        self.bc_params += (
            (self.bc_params_bounds["max"] - self.bc_params_bounds["min"]) / 15 * action
        )
        self.bc_params = jnp.clip(
            self.bc_params, self.bc_params_bounds["min"], self.bc_params_bounds["max"]
        )

        wall_tenperature = get_wall_temperature(self.sim.y, self.bc_params)

        self.sim.update_boundary_condition(
            "left", {"m": wall_tenperature}, "ThermalLattice"
        )

        for _ in range(self.sim_steps_per_env_step):
            self.sim.step()
            self.sim_step += 1
            self.fluid_state = self.sim.get_macroscopics()
    ...
    def _render_frame(self):
        ...
        cmax_img = (
            0.5 * self.gravity * self.thermal_expansion * self.temperature_amplitude
        )
