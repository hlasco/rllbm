...
@jax.jit
def get_bottom_temperature(t, x, nx, freq_x0=0.0005, freq_amp=0.02, amp=0.5, width=0.1):
...
def init_ncfile(path, sim):
    with netCDF4.Dataset(path, "w", format="NETCDF4") as ncfile:
        ...
        ncfile.createDimension("tracer_id", len(sim.tracers))
        ...
        ncfile.buoyancy = buoyancy
        ncfile.prandtl = prandtl
        ncfile.rayleigh = rayleigh
        ncfile.dt = dt


def write_ncfile(path, time_index, time, velocity, temperature, curl):
...
if __name__ == "__main__":
    ...
    #temperature = temperature.at[:, 0].set(get_bottom_temperature(x=sim.y, t=0, nx=ny))
    ...
    sim.initialize(lattice, dfs, tracers=[jnp.array([0.5, 0.5])])
    ...
    for i in trange(80000):
        ...
        if i % 200 == 0:
            ...
            time_index = i // 200
            write_ncfile(nc_path, time_index, sim.time, velocity, temperature, curl)
